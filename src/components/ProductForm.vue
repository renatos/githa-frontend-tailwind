<template>
  <div class="fixed inset-0 z-[1050] bg-slate-900/50 flex items-center justify-center p-4 backdrop-blur-sm" @click.self="$emit('close')">
    <div class="bg-[var(--color-bg-card)] dark:bg-slate-800 rounded-xl shadow-xl w-full max-w-[600px] flex flex-col max-h-[90vh] overflow-hidden border border-slate-200 dark:border-slate-700">
      ...
      <header class="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 px-6 py-4 flex-shrink-0 bg-[var(--color-bg-body)] dark:bg-slate-800/50">
        <div class="flex items-center gap-3">
          <span class="material-symbols-outlined text-[24px] text-slate-900 dark:text-slate-100">inventory_2</span>
          ...
          <div class="overflow-y-auto p-6 flex-1 bg-transparent dark:bg-slate-900/50">
            <div class="flex flex-col gap-6">
              ...
              <div class="flex flex-col md:flex-row gap-6">
                <label class="flex flex-col flex-[3]">
                  <p class="text-slate-900 dark:text-slate-100 text-sm font-medium pb-2">Nome</p>
                  <input id="name" v-model="form.name" class="form-input flex w-full rounded-lg text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 h-11 px-4 text-base transition-colors" type="text" required />
                </label>
              ...
              <label class="flex flex-col">
                ...
                <textarea id="description" v-model="form.description" class="form-input flex w-full rounded-lg text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 p-4 text-base transition-colors" rows="3"></textarea>
              </label>

              <div class="flex flex-col md:flex-row gap-6">
                <label class="flex flex-col flex-1">
                  <p class="text-slate-900 dark:text-slate-100 text-sm font-medium pb-2">Quantidade em Estoque</p>
                  <input id="stockQuantity" v-model="form.stockQuantity" class="form-input flex w-full rounded-lg text-slate-900 dark:text-slate-100 border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-700 focus:border-indigo-600 focus:ring-1 focus:ring-indigo-600 h-11 px-4 text-base transition-colors" type="number" min="0" inputmode="numeric" />
                </label>
